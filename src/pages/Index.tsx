...
const Dashboard = () => {
  ...
  const cumulativeContributionPercentages = contributors?.reduce<{
    name: string;
    start: number;
    end: number;
    amount: number;
  ...
  const cumulativeExpensePercentages = contributors?.reduce<{
    name: string;
    start: number;
    end: number;
    amount: number;
  ...
  const savingsGoal = profile?.savings_goal_percentage ? totalRevenue * profile.savings_goal_percentage / 100 : 0;
  return <DashboardLayout>
      <div className="grid gap-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          <RevenueCard totalRevenue={totalRevenue} contributorShares={cumulativeContributionPercentages} />
          <ExpensesCard totalExpenses={totalExpenses} contributorShares={cumulativeExpensePercentages} />
          <SavingsCard totalMonthlySavings={totalMonthlySavings} savingsGoal={savingsGoal} />
        </div>

        {/* Graphiques circulaires */}
        <div className="grid gap-6 md:grid-cols-2">
          <RecurringExpensesPieChart recurringExpenses={recurringExpenses || []} totalExpenses={totalExpenses} />
