...
/**
 * Page de test pour exécuter manuellement la notification de dépenses
 * Cette page est réservée aux administrateurs
 */
const TestExpenseNotification = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [result, setResult] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);

  const triggerExpenseNotification = async () => {
    setIsLoading(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("notify-expenses-monthly", {
        body: {},
      });
      ...
      toast.success("Notification de dépenses exécutée avec succès");
    } catch (err: any) {
  ...
  return (
    <div className="container mx-auto py-10">
      <Card className="mb-10">
        <CardHeader>
          <CardTitle>Test de Notification de Dépenses</CardTitle>
          <CardDescription>
          ...
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-4">
              <p className="text-muted-foreground">
                Cette fonction envoie un email aux utilisateurs qui ont activé la notification de dépenses
                et qui ont des dépenses enregistrées le mois précédent.
              </p>
              
              <Button 
                onClick={triggerExpenseNotification} 
                disabled={isLoading}
                className="w-fit"
              >
                {isLoading ? (
...
export default TestExpenseNotification;
