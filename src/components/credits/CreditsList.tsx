...
export const CreditsList = ({ credits, onCreditDeleted }: CreditsListProps) => {
  console.log("CreditsList reçoit les crédits :", credits.map(c => ({id: c.id, statut: c.statut})));
  console.log("Objets crédits complets :", credits);
  
  return (
    <div className="grid gap-2">
      {credits && credits.length > 0 ? (
        credits.map((credit) => {
          console.log("Rendu du crédit :", credit.id, "statut:", credit.statut);
          return (
            <Card 
              key={credit.id} 
