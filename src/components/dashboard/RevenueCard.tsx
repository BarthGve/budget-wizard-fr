...
export const RevenueCard = ({
  ...
  return <motion.div initial={{
    ...
    y: -3
      ...
      <Card className={cn("backdrop-blur-sm cursor-pointer transition-all duration-300 hover:shadow-xl",
    // Light mode styles
    "bg-gradient-to-br from-background to-amber-50 shadow-lg border border-amber-100",
    // Dark mode styles
    "dark:bg-gradient-to-br dark:from-gray-900 dark:to-amber-950 dark:border-amber-900/50 dark:shadow-amber-900/10")} onClick={() => navigate("/income")}>
        <CardHeader className="py-4">
          <div className="flex flex-row items-center justify-between">
            <CardTitle className="text-2xl flex items-center gap-2">
              <div className={cn("p-2 rounded-full", "bg-amber-100 text-amber-600",
            // Light mode
            "dark:bg-amber-900/40 dark:text-amber-400" // Dark mode
            )}>
                <Banknote className="h-5 w-5" />
              </div>
              <span className="text-gray-800 dark:text-white">Revenus</span>
            </CardTitle>
          </div>
          <CardDescription className={cn("text-gray-500", "dark:text-gray-400")}>Des contributeurs</CardDescription>
        </CardHeader>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              duration: 0.5
                ...
                <motion.p className={cn("font-bold text-xl leading-none", "text-gray-800",
              // Light mode
