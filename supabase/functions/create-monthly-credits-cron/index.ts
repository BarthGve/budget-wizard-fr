...
serve(async (req) => {
  ...
  try {
    ...
    const functionUrl = `${supabaseUrl}/functions/v1/notify-credits-monthly`;

    // Créer le cron job pour le 1er jour de chaque mois à 8h00
    const { data, error } = await supabase.rpc("pg_cron_schedule", {
      job_name: "monthly_credits_notification",
      schedule: "0 8 1 * *", // À 8h00 le 1er de chaque mois
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: "Cron job pour les notifications mensuelles de crédits créé avec succès",
        data
      }),
      { 
        status: 200, 
